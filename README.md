# ☕ Java Estudos

Repositório criado para acompanhar minha evolução na linguagem Java.

## Fundamentos

- [x] Hello World - [HelloWorld.java](Fundamentals/src/HelloWorld.java)
- [x] Variáveis - [Variables.java](Fundamentals/src/Variables.java)
- [x] Tipos primitivos - [PrimitiveTypes.java](Fundamentals/src/PrimitiveTypes.java)
- [x] Operadores - [Operators.java](Fundamentals/src/Operators.java)
- [x] Entrada de dados (Scanner) - [ScannerExample.java](Fundamentals/src/ScannerExample.java)

## Estruturas de Controle

- [x] if / else - [IfElse.java](ControlFlow/src/IfElse.java)
- [x] switch - [Switch.java](ControlFlow/src/Switch.java)
- [x] Operador ternário - [TernaryOperator.java](ControlFlow/src/TernaryOperator.java)

## Laços de Repetição

- [x] while - [WhileLoop.java](Loops/src/WhileLoop.java)
- [x] do-while - [DoWhileLoop.java](Loops/src/DoWhileLoop.java)
- [x] for - [ForLoop.java](Loops/src/ForLoop.java)
- [x] foreach - [Foreach.java](Loops/src/Foreach.java)

## Métodos

- [x] Criação de métodos - [MethodCreate.java](Methods/src/MethodCreate.java)
- [x] Parâmetros - [MethodParameters.java](Methods/src/MethodParameters.java)
- [x] Retorno - [MethodReturn.java](Methods/src/MethodReturn.java)
- [x] Sobrecarga - [MethodOverloading.java](Methods/src/MethodOverloading.java)

## Arrays

- [x] Arrays - [ArraysBasics.java](Arrays/src/ArraysBasics.java)
- [x] Matrizes - [MatrixBasics.java](Arrays/src/MatrixBasics.java)

## Orientação a Objetos

- [x] Classes e Objetos - [Main.java](ObjectOriented/src/Main.java), [User.java](ObjectOriented/src/User.java)
- [x] Construtores - [User.java](ObjectOriented/src/User.java)
- [x] Encapsulamento - [Main.java](ObjectOriented/src/Main.java), [User.java](ObjectOriented/src/User.java)
- [x] Herança - [Admin.java](ObjectOriented/src/Admin.java)
- [x] Polimorfismo - [Main.java](ObjectOriented/src/Main.java)
- [x] Abstração - [User.java](ObjectOriented/src/User.java), [Developer.java](ObjectOriented/src/Developer.java)
- [x] Interfaces - [Developable.java](ObjectOriented/src/Developable.java), [User.java](ObjectOriented/src/User.java), [Developer.java](ObjectOriented/src/Developer.java)

## Tratamento de Exceções

- [x] try/catch - [TryCatch.java](ExceptionHandling/src/TryCatch.java)
- [x] throw - [Throw.java](ExceptionHandling/src/Throw.java)
- [x] throws - [Throws.java](ExceptionHandling/src/Throws.java)

## Coleções

- [x] ArrayList - [ArrayLists.java](Collections/src/ArrayLists.java)
- [x] LinkedList - [LinkedLists.java](Collections/src/LinkedLists.java)
- [x] HashSet - [HashSets.java](Collections/src/HashSets.java)
- [x] HashMap - [HashMaps.java](Collections/src/HashMaps.java)

## Recursos Modernos

- [x] EnumExample - [EnumExample.java](ModernResources/src/EnumExample.java)
- [ ] Records
- [ ] Generics
- [ ] Lambda
- [ ] Stream API
- [ ] Optional
